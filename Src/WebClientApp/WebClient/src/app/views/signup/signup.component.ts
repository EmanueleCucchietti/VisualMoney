...
export class SignupComponent {
  ...
  signup() {
    ...
    let signupRequestDto: SignupRequestDto = new SignupRequestDto(user, this.password);
  ...
  validateStep0() {
    ...
    return emailRegex.test(this.email) && alphanumericRegex.test(this.username);
  }
  validateStep1() {
    const alphanumericRegex: RegExp = /^[a-zA-Z0-9]+$/;

    return alphanumericRegex.test(this.name) && alphanumericRegex.test(this.surname);
  }
  validateStep2() {
